...
constexpr double OBSTACLE_THRESHOLD {700.0};        // Threshold for detecting obstacle
...
public:
    TaskManager(std::string name, std::string rName, int tStep, std::string mode = "start")
        ...
        mSTModeStages { {2250.0,2250.0}, {705, -705}, {3700.0, 3700.0}, {705, -705}, {3400.0, 3400.0}, {705, -705}, {9400.0, 9400.0}, {-570, 570}, {3200.0, 3200.0}, {-715, 715}, {3400.0, 3400.0}, {-260, 260}, {2600, 2600} }, 
    ...
    std::vector<double> getSTModeSpeeds(); //Calculates the speed at which to run each motor in shortest-time mode
...
std::vector<double> TaskManager::getSTModeSpeeds() {
  ...
  double lGapSpeed = (double)((lRotationTarget - mRotations[0]) * 2 * PI / mTimeStep); //Calculate speed required to bridge the gap between left motor's completed and target rotations
  double rGapSpeed = (double)((rRotationTarget - mRotations[1]) * 2 * PI / mTimeStep); //Calculate speed required to bridge the gap between right motor's completed and target rotations
  if (lRotationTarget > 0 && mRotations[0] < lRotationTarget) { //Set left motor speed according to difference between completed rotations and target rotations
    speeds[0] = std::min(FAST_SPEED, lGapSpeed);
  } else if (lRotationTarget < 0 && mRotations[0] > lRotationTarget) {
    speeds[0] = std::max(-FAST_SPEED, lGapSpeed);
  }

  if (rRotationTarget > 0 && mRotations[1] < rRotationTarget) { //Set right motor speed according to difference between completed rotations and target rotations
    speeds[1] = std::min(FAST_SPEED, rGapSpeed);
  } else if (rRotationTarget < 0 && mRotations[1] > rRotationTarget) {
    speeds[1] = std::max(-FAST_SPEED, rGapSpeed);
  }
...
void CsvProcessor::bProcessData(std::vector<std::vector<double> > dataMatrix) const {
  ...
  std::vector<double> ds0Col;
  std::vector<double> ds1Col;
  std::vector<double> ds2Col;
  std::vector<double> ds3Col;
  std::vector<std::vector<double> > columns;
  columns.push_back(timeCol);
  columns.push_back(ds0Col);
  columns.push_back(ds1Col);
  columns.push_back(ds2Col);
  columns.push_back(ds3Col);

  int nRows = dataMatrix.size();

  for (auto row = dataMatrix.begin(); row != dataMatrix.end(); row++) { //Iterate through rows and columns of data matrix
      for (auto col = row->begin(); col != row->end(); col++) {
        int index = col - row->begin();
        columns[index].push_back(*col); //Push each value in each row to the corresponding column vector
      }
  }

  for (auto iter = columns.begin(); iter != columns.end(); iter++) { //Sort each column
  ...
  if (fout.is_open()) {  // check if open successfully
    for (int i = 0; i < nRows; i++) { //Write each row of sorted values to the output file
        for (auto iter = columns.begin(); iter != columns.end(); iter++) {
          fout << std::fixed << std::setprecision(3) << (*iter)[i] << mDelim;
        }      
...
int main(int argc, char **argv) {
  ...
  while (robot.step(timeStep) != -1) {
    ...
    bool wallRight = dsValues[0] > OBSTACLE_THRESHOLD;
    bool wallLeft = dsValues[3] > OBSTACLE_THRESHOLD;
    bool wallAhead = dsValues[1] > OBSTACLE_THRESHOLD || dsValues[2] > OBSTACLE_THRESHOLD;
    ...
    if (mode == "man") { //Manual control mode
      ...
      if (robotName == "ROBOT_RED") {
        ...
        csvProcessor.bProcessData(csvProcessor.readDataFromCsv());
      }
      ...
      std::vector<double> motorSpeeds = tMan.getSTModeSpeeds();
