...
public:
    ...
    std::vector<double> getSTModeSpeeds(bool wallAhead); //Calculates the speed at which to run each motor in shortest-time mode
...
std::vector<double> TaskManager::getSTModeSpeeds(bool wallAhead) {
  ...
  bool leftCorrect = wallAhead && mSTModeWalls[mSTModeIndex] == 'l';
  bool rightCorrect = wallAhead && mSTModeWalls[mSTModeIndex] == 'r';
  ...
  speeds[0] = std::min((mRotations[0] > 0 || leftCorrect) * ((mRotations[0] < 0 || rightCorrect) * -1) * FAST_SPEED, lGapSpeed);
  if (leftCorrect) {

  } else if (lRotationTarget > 0 && mRotations[0] < lRotationTarget) { //Set left motor speed according to difference between completed rotations and target rotations
  ...
  speeds[1] = std::min((mRotations[1] > 0 || rightCorrect) * ((mRotations[1] < 0 || leftCorrect) * -1) * FAST_SPEED, rGapSpeed);
  if (rightCorrect) {
    speeds[1] = FAST_SPEED;
  } else if (leftCorrect) {
    speed[1] = -FAST_SPEED;
  } else if (rRotationTarget > 0 && mRotations[1] < rRotationTarget) { //Set right motor speed according to difference between completed rotations and target rotations
